...
`fake-style-loader` adds a property to the standard css-modules interface `source: String`
...
All options are available only when the parameter `modules` set to `true` (see options (css-loader)[https://github.com/webpack-contrib/css-loader#options])
